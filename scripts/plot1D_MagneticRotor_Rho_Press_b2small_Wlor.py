...
def plot_rho(rho, ax, lb):
  ...
  ax.plot(x, rho, '-', color= 'k', lw=0.8, alpha=0.9)
  if lb: 
     ax.set_ylabel(r'$\rho $', fontsize=6.5)
#  else: 
#     ax.set_yticks([])
  ax.set_ylim(0, 7.5)
...
def plot_wlor(wlor, ax, lb):
  ...
  ax.xaxis.set_minor_locator(minorLocator)
  ax.xaxis.set_major_locator(majorLocator)
  minorLocator = ticker.MultipleLocator(0.025)
...
def plot_press(press, ax, lb):
 ...
 # ax.xaxis.set_minor_locator(minorLocator)
 # ax.xaxis.set_major_locator(majorLocator)
  minorLocator = ticker.MultipleLocator(5e-2)
  majorLocator = ticker.MultipleLocator(1e-1)
  #ax.yaxis.set_minor_locator(minorLocator)
...
def main(runs):
  for (dr, str_it, it) in runs:
    ...
    print("\nDone plotting!")
    plt.savefig(home+"/Desktop/1D_MagRot.pdf", bbox_inches = 'tight', pad_inches = 0.03)
